...
const campaigns = [
    {
        ...
        "type": "🥰 😈 Kinky Wet Sexting with me😋💦", "profit": "$ 1000", "costEarnings": "$ 10000", "affiliate": "MAX",
    },
    {
        "label": "TAGA 022",
        "status": "Active", "revenue": "$ 1000",
...
export const CampaignTable = () => {
    ...
    const columns: ColumnDef<Campaign>[] = [
                ...
                return (
                    <div className='flex items-center gap-2'>
                        <div className={'flex flex-col'}>
                            <p>{info.getValue()}</p>
                            <p className='text-[12px] text-gray'>  {new Date(row.startDate).toLocaleDateString()} -
                                {new Date(new Date(row.startDate).getTime() + row.duration * 24 * 60 * 60 * 1000).toLocaleDateString()}</p>
                        </div>
                        <a href={row.url} target="_blank">
                            <Link color='#248BDA' className='w-4'/></a>
                    </div>
        ...
        },
        {
            ...
            cell: (info) => (<p className='max-w-[200px] text-wrap'>{info.getValue()}</p>),
        },
        {
